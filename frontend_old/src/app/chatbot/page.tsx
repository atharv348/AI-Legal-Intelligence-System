...
export default function ChatbotPage() {
  const [messages, setMessages] = useState<Message[]>([
    { role: "bot", content: "Namaste. I am Nyaya AI, your trusted legal assistant. I can help you understand Indian law, your fundamental rights, and guide you through legal procedures. How can I help you today?" }
  ]);
  ...
  return (
    <div className="max-w-5xl mx-auto py-6 px-4 flex flex-col h-[calc(100vh-4rem)]">
      {/* Header Area */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4 bg-white dark:bg-[#1E293B] p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-[#5D5346]/10 flex items-center justify-center border border-[#5D5346]/20">
            <MessageSquare className="text-[#5D5346]" size={24} />
          </div>
          <div>
            <h1 className="text-2xl font-black text-slate-900 dark:text-white">Nyaya Citizen Chatbot</h1>
            <div className="flex items-center gap-2 mt-1">
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-[10px] font-bold text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800 uppercase tracking-wider">
                <ShieldCheck size={10} /> Nyaya AI v3.0 Verified
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 bg-zinc-50 dark:bg-zinc-950 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 flex flex-col overflow-hidden shadow-inner relative">
              ...
              <div className={`flex gap-4 max-w-[85%] md:max-w-[70%] ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                ...
                <div className={`group relative p-5 rounded-3xl text-sm leading-relaxed shadow-sm transition-all hover:shadow-md ${
                  ...
                  <div className={`absolute bottom-[-1.5rem] ${msg.role === "user" ? "right-0" : "left-0"} text-[10px] font-bold text-zinc-400 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity`}>
                    {msg.role === "user" ? "Sent" : "Nyaya AI Assistant"}
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Input Area */}
        <div className="p-6 bg-white dark:bg-[#1E293B] border-t border-slate-200 dark:border-slate-800 relative z-20">
          <form onSubmit={handleSend} className="relative max-w-4xl mx-auto group">
            {/* Selected File Preview */}
            {file && (
              <div className="absolute bottom-full mb-4 left-0 right-0 flex items-center gap-3 p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-2xl animate-in slide-in-from-bottom-2">
            ...
            <div className="relative">
              <input
                ...
                placeholder="Ask about your rights or upload a document image..."
                disabled={loading}
                className="w-full pl-6 pr-44 py-5 bg-slate-100 dark:bg-[#0F172A] border border-slate-200 dark:border-slate-700 rounded-[2rem] outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 dark:focus:border-indigo-400 transition-all text-base shadow-inner group-hover:bg-white dark:group-hover:bg-[#0F172A] text-slate-900 dark:text-white placeholder:text-slate-500 dark:placeholder:text-slate-400 font-medium"
              />
              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
                <VoiceInput onTranscript={(t) => setInput(prev => prev + (prev ? " " : "") + t)} />
                <label className="p-2.5 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 cursor-pointer transition-colors rounded-full hover:bg-slate-200 dark:hover:bg-slate-700">
